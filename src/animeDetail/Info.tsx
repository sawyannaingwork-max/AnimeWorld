    ...
    return(
        <div className="mt-9 w-[90%] mx-auto">
            <div className="lg:flex items-start gap-5">
                {
                    anime.images?.jpg?.image_url && <img className="mx-auto rounded-md shadow-md shadow-gray-600" src={anime.images.jpg.image_url} alt={anime.title_english? anime.title_english : "Unknown"} />
                }
                <div className="mt-4 lg:mt-0">
     
                    <div className="lg:flex lg:gap-2">
                        <h1 className="text-center lg:text-left font-albert text-3xl text-primary">{anime.title_english? anime.title_english : "Unknown"}</h1>
                        <div className="mt-2 lg:mx-0 lg:max-w-max lg:mt-0 bg-primary px-2 py-1 rounded-md text-center text-background font-albert max-w-50 mx-auto">
                            Aired {anime.aired.from && `from ${anime.aired.from.split("T")[0]}`} {anime.aired.to && `to ${anime.aired.to.split("T")[0]}`}
                        </div>
                    </div>
                    {
                        anime.title_japanese && 
                        <h2 className="mt-2 lg:text-left text-2xl font-albert text-primary text-center">{anime.title_japanese} <span className="text-secondary">(Japanese)</span></h2>
                    }
            ...
            {
                anime.background && 
                <p className="text-xl text-gray-800 font-albert mt-5 text-center">{anime.background}</p>
            }
            {
                anime.trailer.embed_url &&
                <div className="mt-9">
                    <h2 className="text-center text-2xl font-alice mb-5">Watch Trailer</h2>
                    <iframe src={anime.trailer.embed_url} className="aspect-video max-w-175 mx-auto"></iframe>
                </div>
            }
            {
                anime.synopsis &&
